      ...
      >
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" @submit.prevent="submit">
            ...
            <button
              type="submit"
              class="w-full text-white bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-gray-700 dark:hover:bg-gray-900 dark:focus:ring-gray-800"
            >
              Log in
            </button>
          </form>
...
export default {
  ...
  methods: {
    submit() {
      axios
        .post('http://localhost:8080/', { Username: this.username, Password: this.password })
        .then((res) => {
          console.log(res)
        })
        .catch((error) => {
